    ...
    <form method="GET" class="mb-3 d-flex align-items-center">
        ...
        <select name="kategori" class="form-select me-2">
            ...
            <?php foreach ($categories as $cat): ?>
                <option value="<?= htmlspecialchars($cat['kategori']); ?>" <?= $cat['kategori'] === $kategori ? 'selected' : ''; ?>>
                ...
                <div class="col-1 col-lg-2 col-md-4 mb-4">
                    <a href="detail_product.php?id=<?= $product['id']; ?>" class="card h-100">
                        <?php if (!empty($product['gambar']) && file_exists('uploads/' . $product['gambar'])): ?>
                            <img src="uploads/<?= htmlspecialchars($product['gambar']); ?>" class="card-img-top" alt="<?= htmlspecialchars($product['nama']); ?>">
                        <?php else: ?>
                            <img src="https://via.placeholder.com/150" class="card-img-top" alt="<?= htmlspecialchars($product['nama']); ?>">
                        <?php endif; ?>
                        <div class="card-body">
                            ...
                            <p class="card-text"><strong>Kategori:</strong> <?= htmlspecialchars($product['kategori']); ?></p>
                        </div>
                    </a>
                </div>
            <?php endforeach; ?>
    ...
    <nav aria-label="Pagination Navigation" class="mt-5">
        <ul class="pagination justify-content-center">
            ...
            <?php if ($current_page > 1): ?>
                <li class="page-item">
                    <a class="page-link" href="?page=<?= $current_page - 1; ?>&search=<?= urlencode($search); ?>&kategori=<?= urlencode($kategori); ?>" aria-label="Previous">
            ...
            if ($start_page > 1): ?>
                <li class="page-item">
                    <a class="page-link" href="?page=1&search=<?= urlencode($search); ?>&kategori=<?= urlencode($kategori); ?>">1</a>
                </li>
                <?php if ($start_page > 2): ?>
            ...
            if ($end_page < $total_pages): ?>
                ...
                <?php endif; ?>
                <li class="page-item">
                    <a class="page-link" href="?page=<?= $total_pages; ?>&search=<?= urlencode($search); ?>&kategori=<?= urlencode($kategori); ?>"><?= $total_pages; ?></a>
                </li>
            <?php endif; ?>
            ...
            <?php if ($current_page < $total_pages): ?>
                <li class="page-item">
                    <a class="page-link" href="?page=<?= $current_page + 1; ?>&search=<?= urlencode($search); ?>&kategori=<?= urlencode($kategori); ?>" aria-label="Next">
